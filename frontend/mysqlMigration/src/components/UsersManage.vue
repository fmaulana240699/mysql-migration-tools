  ...
  <table>
    ...
    <tbody>
      <tr v-for="user in users" :key="user.id">
          ...
          <td> {{ user.username }}  </td>
          <td> {{ user.role }}  </td>
          <td> <button @click="editUser(user.id)"> <svg width="35px" height="35px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <td> <button @click="deleteUser(user.id)"><svg width="35px" height="35px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  if (result.isConfirmed) {
    try {
      ...
      if (response.status === 200 | response.status === 204) {
        this.$swal('Deleted!', 'Users file has been deleted.', 'success');
